...
// FIFA World Cup 26 Annex C — combinations for eight best third-placed teams.
// Source: FWC2026 regulations, Annex C (495 rows = C(12,8)).
...
/**
 * Map each fixed group-winner slot (1A, 1B, 1D, …) to the assigned third-placed team.
 * Returns null when the eight qualifiers cannot be determined yet.
 */
export function buildThirdPlaceByWinner(standingsByGroup) {
  const ranking = rankThirdPlaceTeams(standingsByGroup);
  const qualified = ranking.filter((entry) => entry.qualifies);
  ...
  const byGroup = Object.fromEntries(qualified.map((entry) => [entry.group, entry]));
  const result = {};

  for (let i = 0; i < ANNEX_C_WINNERS.length; i += 1) {
    const winnerGroup = ANNEX_C_WINNERS[i];
    const thirdGroup = byWinner[winnerGroup];
    const entry = byGroup[thirdGroup];

    if (entry?.team) {
      result[winnerGroup] = { team: entry.team, group: entry.group };
    }
  }

  return Object.keys(result).length === ANNEX_C_WINNERS.length ? result : null;
}

export function formatThirdPlaceSelectionMessage(groups) {
  return `This team was selected among the best third-placed teams from groups ${groups.join(', ')}.`;
}
